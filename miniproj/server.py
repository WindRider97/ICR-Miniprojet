...
class Server():
    ...
    def upload_data(self, user):
        self.users_keys[user.uid], self.users_shared_keys[user.uid], self.users_folders_mapping[user.uid], self.users_shared_mapping[user.uid], self.users_enc_folder_mapping[user.uid] = user.upload_data()
        
    def download_data(self, user):
        user.download_data(self.users_keys[user.uid], self.users_shared_keys[user.uid], self.users_folders_mapping[user.uid], self.users_shared_mapping[user.uid], self.users_enc_folder_mapping[user.uid])
...
if __name__ == '__main__':
    ...
    alice.fetch_shared_folder(folder_uid, server.users_enc_folder_mapping[bob.uid])
    alice.fetch_shared_folder(folder_uid2, server.users_enc_folder_mapping[bob.uid])
